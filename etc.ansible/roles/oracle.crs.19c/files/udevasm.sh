...
[[ $DEVPATH =~ /target([1-9]):[0-9]:([0-9]) ]] && printf "ASMNAME=asmshared0%02d%s" ${BASH_REMATCH[2]} "${PART}" >> /tmp/udevtest2.out
[[ $DEVPATH =~ /target([1-9]):[0-9]:([0-9]) ]] && printf "ASMNAME=asmshared0%02d%s" ${BASH_REMATCH[2]} "${PART}"
